...
namespace DocuArchi.Api.Controllers.GestionCorrespondencia.Firmas
{
    ...
    [ApiController]
    public sealed class SolicitaListaFirmasAutorizadasDocumentoController : ControllerBase
    {
        private readonly IClaimValidationService _claimValidationService;
        private readonly IServiceSolicitaListaFirmasAutorizadasDocumento _service;

        public SolicitaListaFirmasAutorizadasDocumentoController(
            IClaimValidationService claimValidationService,
            IServiceSolicitaListaFirmasAutorizadasDocumento service)
        {
            _claimValidationService = claimValidationService;
            _service = service;
        }

        [HttpGet("autorizadas-documento")]
        public async Task<ActionResult<AppResponses<List<ResponseDropdownDto>>>> Get([FromQuery] long idUsuarioAutorizado)
        {
            ...
            {
                return BadRequest(Validation("idUsuarioAutorizado", "IdUsuarioAutorizado requerido"));
            }

            var result = await _service.SolicitaListaFirmasAutorizadasDocumentoAsync(
                idUsuarioAutorizado,
                usuarioId,
