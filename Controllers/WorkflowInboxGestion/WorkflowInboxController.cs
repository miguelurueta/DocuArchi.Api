...
namespace DocuArchi.Api.Controllers.WorkflowInboxGestion
{
    ...
    public sealed class WorkflowInboxController : Controller
    {
        ...
        public async Task<ActionResult<AppResponses<WorkflowInboxAutocompleteResponseDto>>> AutocompleteBandejaWorkflow(
            [FromBody] WorkflowInboxAutocompleteRequestDto request)
        {
            ...
            var result = await _workflowInboxService.AutocompleteBandejaWorkflowAsync(
                request,
                idUsuarioGestion,
                validation.ClaimValue);
            if (!result.success)
